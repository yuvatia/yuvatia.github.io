...
TODO railway track at an angle
...
TODO one-point perspective cube
...
TODO two-point perspective cube
...
TODO three-point perspective cube
...
1. Top-down view - the plane in which the track lies is parallel to the image plane, thus the track is represented as a rectangle in our drawing because all lines are parallel to the image plane. However, in $P^{3}$, the edges do in fact still intersect at a point at infinity. The horizon line in this case is just not visible when looking at the picture plane.
...
TODO three-point perspective with lines at infinity
...
TOOD two-point perspective with lines at infinity
...
TODO one-point perspective with lines at infinity
...
TODO shadow of triangle puzzle
...
TODO axis and point of perpsecitvity in our puzzle
...
TODO tiled floor with points XPY
...
We would like to extend the concept of being "in perspective from a point" beyond triangles. Consider two planes $\pi$, $\pi'$ in $P^3$ and some point $O$ not incident on either plane. A map $T: \pi \to \pi'$ is called a **perspectivity centered at $O$** if $\forall x \in \pi: $ $x, T(x)$ and $O$ are collinear. Consider the lamp post drawing from earlier:

TODO perspectivity such that $\pi'$ is the shadow of $\pi$
...
TODO persepctivity similar to lamp post but without a triangle (now on ground) and with an eye in place of the lamp
...
We see that perspectivity maps can be interpreted as shadows and as images formed by the camera or the eye. These interpretations provide a good intuition, but as we have stated before they can be quite misleading when dealing with projective spaces since they still make describing FOO FOO FOO
...
Clearly, this construction satisfies our requirement. It is illustrated by the following illustration:

TODO illustration AB to A prime B prime 1d
...
TODO illustration ftpg(2d)

### Existence

Consider the lines $AB$, $CD$. The lines are coplanar and distinct so they are incident at some point $E$. Similarly, consider $A'B'$ and $C'D'$, which are incident at some point $E'$ by the same argument. By the 1D version of the fundemental theorem, we know that there exists a projectivity from $CD$ to $C'D'$ taking $C, D, E$ to $C', D', E'$, and while the 1D projectivity is unique (in the sense that the map from $CD$ to $C'D'$ under these conditions is unique), the same is not true with regards to where the projectivity takes the rest of the plane. It is trivial to find one such perspectivity from $\pi$ to some plane $\pi^{*}$ such that $\pi^{*} \ne \pi'$, which would of course contain the line through $C'D'$. This projectivity takes $A, B$ to points $A^{*}, B^{*} \in \pi^{*}$. However since projectivities preserve incidence relations we have $A^{*}B^{*}$ incident with $C'D'$ at $E'$. Since $A'B'$ and $A^{*}B^{*}$ are incident at $E'$ they are coplanar and we can construct a perspectivity from $\pi$ to $\pi'$ centered at $A^{*}B' \cap A'B^{*}$, as illustrated:

TODO existence from pi to pi prime

Notice that by the definition of a perspectivity, the line over which the planes intersect is fixed, thus the perspectivity takes $A^{*}$ to $A'$, $B^{*}$ to $B'$, $C'$ to $C'$ and $D'$ to $D'$. By composing the projectivity from $\pi$ to $\pi^{*}$ with this perspectivity from $\pi^{*}$ to $\pi'$, we get a projectivitiy $\pi \to \pi'$ such that $\{A, B, C, D\}$ gets taken to $\{A', B', C', D'\}$, thus showing the existence of such projectivity.
...
TODO four fixed points theorem setup
...
TODO dual notation of cross ratio sketch
...
TODO perspectivity ground plane until parallel
...
In $RP^{2}$, we're dealing with projective points which are really lines in $R^{3}$, and projective lines which are planes in $R^{3}$. This notation readily describes the perspectivity itself, but our primary objective is to describe perspective shifts in terms of $R^{3}$ - we would like to define a map from $RP^{3}$ to $R^{3}$ which takes projective points to points and projective lines to lines. While the actual map is fairly intuitive to understand, I would like to be provide some more insight into the algebraic structures that govern it.

### Affine Spaces

TODO side quest to affine spaces


ffine chart
PGL(3, R) SO(3) -> Aff -> PGL(3, R)
Note on Aff group and the properties preserved (convexity, parallelism, ratios...)
homography + Fundemental theorem of PGL(3, R)
orthographic vs perpsective
central projection matrix in 3D graphics
